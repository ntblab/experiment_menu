...
%
% Takes as an input a calibration filename (created by the
% `Scripts/Screen_calibration/WarpingUndistortionDemo.m` script) and sets
% up the screen based on this filename. This means the projector distortion
% is corrected for. If one monitor is found then you can either show it
% transparently (thus making output to the command window visible) or it
% can show the whole screen, or it can be shown whole screen but with the
% text being outputted to the screen
%
% To set the screen up, use the WarpingUndistortionDemo.m script in
% Screen_calibration/ to set up the distortion of the display, which can
% deal with stretching, curvature and keystoning. These calibration files
% also have details about the size of the display and the viewing distance
% of the participant.
%
% It is possible to make a 'calibration' file that has only the screen
% width (`Screen_width`) and viewing distance (`Viewing_dist`) variables
% specified and then putting that in the Screen_calibration folder to make
% it accessible
%
...
% not.
%
% Description added: 3/9/16 C Ellis Changed script to search the calib
% directory for scripts with appropriate information
