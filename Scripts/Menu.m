...
% experiments
% 
% This function sets up the display necessary for the projector or
% screen set up, the eye tracker and the response devices (including a
% scanner). Then it has you pick an experiment, generates the trials for
% the experiment (if they have not already been generated), and then runs a
% block from that experiment. The program then cycles back, having you
% either run another block of that experiment, reselecting an experiment
% from the menu or automatically advancing to the next block
%
...
Savelocation='../Data/';
EyeTracking_Experiments={'Experiment_MemTest', 'Experiment_PosnerCuing', 'Experiment_FPTest'}; % List the experiments which need calibration. If an experiment is in this list then it will encourage you to do calibration after you have run this for the first time (useful for offline eye coding, useless for online coding)
...
while QuitMenu==0
                    ...
                    if JumpToVideo==1
                        ...
                        if BlockNumber>1
                            ...
                            for BlockCounter=1:BlockNumber
                                ...
                                %Put an X in front of a block for every time it has been completed
                                ...
                                PrintText_List=Utils_PrintText(Window, PrintText_List,  sprintf('\n\n %s   %0.0f: %s', Prefix, BlockCounter, GenerateTrials.(SelectedExperimentName).Parameters.BlockNames{BlockCounter}));
                            end
    ...
    if strcmp(Str, 'q')
        ...
        %Check that the block has not been run yet. If there is an error
        %then assume that the block doesn't exist. Name the block accordingly
        try
            ...
            for StructCounter=1:length(Fields)
                ...
                FieldBlock=str2num(Fields{StructCounter}(7:6+length(num2str(ChoosenBlock))));
